    ...
    <section id="agenda" className="section section-alt">
      <div className="section-head section-head-wrap">
        ...
        <div className="season-switch">
          <button type="button" className={`season-btn${season === 's1' ? ' active' : ''}`} onClick={() => setSeason('s1')}>2026 — 2027</button>
          <button type="button" className={`season-btn${season === 's2' ? ' active' : ''}`} onClick={() => setSeason('s2')}>2027 — 2028</button>
        </div>
      </div>
      <div className="agenda-list">
        {dates.map((d) => (
          <div className="agenda-row" key={d.date + d.city}>
